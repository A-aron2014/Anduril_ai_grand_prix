...
class DrawAircraftWithTargets:
    ...
    def update(self, time, aircraft_state,
        ...
        # --- aircraft body (3D only) ---
        if self._ax is not None and self._handles:
            self._update_bodies(pn, pe, pd, phi, theta, psi)
            self._update_axis(pe, pn, pd)

        # --- FOV circle — handled in _update_inset for 2D mode;
        #     only call the 3D version if axes exist ---
        if self._ax is not None and self._fov_fn is not None:
            self._update_fov(pos_for_fov)

        # --- targets (3D dots + 2D inset) ---
        if targets:
            self._update_targets(targets, found_ids)

        # --- orbit circle (3D only) ---
        if self._ax is not None and gsm is not None:
    ...
    def _init_figure(self, pn, pe, pd, phi, theta, psi):
        # Use figure 30 to avoid colliding with figures 8,9,11,12,20,21
        # already created in the plotting section of run()
        self._fig = plt.figure(30, figsize=(12, 8))
        self._fig.clf()
        plt.close(20)   # close old figure 20 if it exists from DrawAircraft
    ...
    def _update_inset(self, pn, pe, pos_for_fov, targets, found_ids, gsm):
        ...
        self._ac_history_e.append(pe)
        self._ac_history_n.append(pn)

        #Limit length
        max_len = self._trail_len
        self._ac_history_e = self._ac_history_e[-max_len:]
        self._ac_history_n = self._ac_history_n[-max_len:]
        # --- FOV circle on 2D inset (works even without 3D axes) ---
        if self._fov_fn is not None:
            origin, r = self._fov_fn(pos_for_fov)
            if self._inset_fov is None:
                self._inset_fov = plt.Circle(
                    (origin[1], origin[0]), r,
                    color='cyan', alpha=0.25, fill=True, linewidth=1.2,
                    linestyle='-', zorder=3
                )
                self._ax2d.add_patch(self._inset_fov)
            else:
        ...
        # --- Orbit circle on 2D inset ---
        if gsm is not None:
            from GuidanceStateMachine import GuidanceState
            if gsm.state in (GuidanceState.SEARCH, GuidanceState.TRACK):
                center = gsm.search_orbit_center if gsm.state == GuidanceState.SEARCH \
                         else gsm.track_center
                radius = gsm.search_orbit_r if gsm.state == GuidanceState.SEARCH \
                         else gsm.p.track_r
                col = 'orange' if gsm.state == GuidanceState.SEARCH else 'magenta'
                if self._inset_orbit is None:
                    self._inset_orbit = plt.Circle(
                        (center[1], center[0]), radius,
                        color=col, alpha=0.15, fill=True,
                    ...
                    self._inset_orbit.set_color(col)
                    self._inset_orbit.set_visible(True)
            elif self._inset_orbit is not None:
                self._inset_orbit.set_visible(False)

        # --- State label as 2D text on inset ---
        if gsm is not None:
            label = f"State: {gsm.state_name}  t={gsm.state_log and len(gsm.state_log)*0.1:.0f}s"
            if self._state_text is None:
                self._state_text = self._ax2d.text(
                    0.02, 0.97, label,
                    transform=self._ax2d.transAxes,
                    fontsize=8, verticalalignment='top',
        ...
        #--- aircraft trail ---
        # if self._inset_ac_trail is None:
        #     (self._inset_ac_trail,) = self._ax2d.plot(
        #         self._ac_history_e,
        #         self._ac_history_n,
        #         '-',
        #         linewidth=1.5,
        ...
        #     )
        # else:
        #     self._inset_ac_trail.set_data(
        #         self._ac_history_e,
        #         self._ac_history_n
        #     )

        # --- aircraft trail ---
        if len(self._ac_history_e) > 1:
            points = np.array([self._ac_history_e, self._ac_history_n]).T.reshape(-1,1,2)
            ...
            lc.set_color([(0,0,0,a) for a in alphas])

            # Remove old trail
            if self._inset_ac_trail is not None:
                self._inset_ac_trail.remove()

            # Add new
            self._inset_ac_trail = self._ax2d.add_collection(lc)

        # Auto-zoom inset around aircraft + targets
